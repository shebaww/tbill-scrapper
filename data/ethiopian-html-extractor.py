...
MASTER_CSV = "ethiopian-tbills.csv"
def main():
  mega_list = get_tables()
  result = divide_tables(mega_list)
  clean_table = parse_list(result)
  write_tocsv(clean_table)
...
def get_tables():
  ...
  with open("NBE.gov.html") as file:
    content = file.read()
...
def write_tocsv(clean_table):
    ...
    for df in clean_table:
      ...
      file_exists = True
def divide_tables(mega_list):
    result_tables = []
    seen_auctions = set()
    announcement_tables = []
    for table in mega_list:
...
def parse_list(result):
  clean_table = []
  for table in result:
    cleaned = clean_info(table)
    if len(cleaned) < 2:
      continue
    clean_table.append({
        "rows": cleaned,
        "issue_number": get_issue_number(table),
        "issue_date": get_issue_date(table),
    })
